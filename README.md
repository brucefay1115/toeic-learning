...
### ☁️ Google 雲端備份
登入 Google 帳號即可將學習紀錄、單字本**自動備份到 Google Drive**，換裝置也不怕遺失進度（不包含 API Key）。
...
### Step 1 — 設定 API Key
首次開啟會自動彈出設定視窗，點選連結前往 Google AI Studio 取得免費 API Key，貼上後儲存。

### Step 2 — 選擇難度與主題
切換到「練習」頁籤，選擇你的目標分數（500–900），可選填自訂主題（如：機場報到、商業會議、餐廳點餐），點擊「開始學習」。
...
### Step 4 — 儲存單字，開啟 SRS 複習
看到不熟的單字？按下書籤圖示儲存到單字本。累積 3 個以上單字後，即可在「單字本」頁籤啟動 SRS 複習，透過三種題型反覆練習直到真正記住。
...
| 雲端備份 | Google Drive 自動同步，跨裝置無縫銜接 |
